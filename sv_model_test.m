...
% ---- residSD: the simulator's own noise, on the response scale ----------
mreport('residSD is zero for a deterministic fit', obj.residSD == 0, ...
    'nugget 0 means no simulator noise to report');

fn = sv_fit(Xtr, ytr, 'm', 15, 'nu', 2.5, 'nugget', 0.05, 'vcf', false);
pn = fn.model.parms;
mreport('residSD is sqrt(variance * nugget)', ...
    abs(fn.residSD - sqrt(pn(1)*pn(end))) < 1e-15 && fn.residSD > 0, ...
    sprintf('residSD %.5f from variance %.4f, nugget %.3f', ...
    fn.residSD, pn(1), pn(end)));

% Tie it to something independent: noise_free drops exactly this much
% variance, so the gap between the two predictions is residSD^2.
vf = sv_predict(fn.model, xq, 'm', 100, 'joint', false, 'variance', true, ...
    'noise_free', true);
vn = sv_predict(fn.model, xq, 'm', 100, 'joint', false, 'variance', true, ...
    'noise_free', false);
gap = vn.var - vf.var;
mreport('residSD^2 is the variance a noisy prediction adds back', ...
    max(abs(gap - fn.residSD^2)) < 1e-12, ...
    sprintf('gap %.6f vs residSD^2 %.6f', mean(gap), fn.residSD^2));
