...
report('pointwise prediction variance equals exact (noisy)', ...
    max(abs(ps.var - diag(Cov_ex))) < 1e-8, ...
    sprintf('max diff %.3e', max(abs(ps.var-diag(Cov_ex)))));

% noise_free (the default) predicts the latent process: the target's own
% nugget is excluded, so the variance drops by exactly sigma^2*tau^2
Kpp_lat = Kpp - parms(1)*parms(end)*eye(np);
var_lat = diag(Kpp_lat - Kpo*(Koo\Kpo'));
pnf = sv_predict(fit, locp, 'm', n, 'joint', false, 'X_pred', Xp, 'variance', true);
report('noise-free variance equals latent-process kriging', ...
    max(abs(pnf.var - var_lat)) < 1e-8, ...
    sprintf('max diff %.3e', max(abs(pnf.var - var_lat))));
report('noise_free reduces the variance by exactly sigma^2*tau^2', ...
    max(abs((ps.var - pnf.var) - parms(1)*parms(end))) < 1e-10, ...
    sprintf('gap %.6f vs %.6f', mean(ps.var-pnf.var), parms(1)*parms(end)));
